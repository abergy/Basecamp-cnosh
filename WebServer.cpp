/*
   ...
   (https://www.ct.de) Licensed under GPLv3. See LICENSE for details.
   */
...
void WebServer::begin(Configuration &configuration,
    ...
    server.on(
        "/configuration.htm", HTTP_GET, [](AsyncWebServerRequest *request) {
            AsyncWebServerResponse *response =
                request->beginResponse_P(200, "text/html", configuration_htm_gz,
                                         configuration_htm_gz_len);
            response->addHeader("Content-Encoding", "gzip");
            request->send(response);
        });
    server.on(
        "/feeding_times.htm", HTTP_GET, [](AsyncWebServerRequest *request) {
            AsyncWebServerResponse *response =
                request->beginResponse_P(200, "text/html", feeding_times_htm_gz,
                                         feeding_times_htm_gz_len);
            response->addHeader("Content-Encoding", "gzip");
            request->send(response);
        });
    server.on(
        "/profiles_add.htm", HTTP_GET, [](AsyncWebServerRequest *request) {
            AsyncWebServerResponse *response = request->beginResponse_P(
                200, "text/html", profiles_add_htm_gz, profiles_add_htm_gz_len);
            response->addHeader("Content-Encoding", "gzip");
            request->send(response);
        });
    server.on("/profiles_cat_amount.htm", HTTP_GET,
              [](AsyncWebServerRequest *request) {
                  AsyncWebServerResponse *response = request->beginResponse_P(
                      200, "text/html", profiles_cat_amount_htm_gz,
                      profiles_cat_amount_htm_gz_len);
                  response->addHeader("Content-Encoding", "gzip");
                  request->send(response);
              });
    server.on("/profiles_cat_rfid.htm", HTTP_GET,
              [](AsyncWebServerRequest *request) {
                  AsyncWebServerResponse *response = request->beginResponse_P(
                      200, "text/html", profiles_cat_rfid_htm_gz,
                      profiles_cat_rfid_htm_gz_len);
                  response->addHeader("Content-Encoding", "gzip");
                  request->send(response);
              });
    server.on("/profiles.htm", HTTP_GET, [](AsyncWebServerRequest *request) {
        AsyncWebServerResponse *response = request->beginResponse_P(
            200, "text/html", profiles_htm_gz, profiles_htm_gz_len);
        response->addHeader("Content-Encoding", "gzip");
        request->send(response);
    });
    server.on("/statistics.htm", HTTP_GET, [](AsyncWebServerRequest *request) {
        AsyncWebServerResponse *response = request->beginResponse_P(
            200, "text/html", statistics_htm_gz, statistics_htm_gz_len);
        response->addHeader("Content-Encoding", "gzip");
        request->send(response);
    });
    ...
    server.on("/main.css", HTTP_GET, [](AsyncWebServerRequest *request) {
        AsyncWebServerResponse *response = request->beginResponse_P(
            200, "text/css", main_css_gz, main_css_gz_len);
        response->addHeader("Content-Encoding", "gzip");
        request->send(response);
    });
    ...
    server.on("/main.js", HTTP_GET, [](AsyncWebServerRequest *request) {
        AsyncWebServerResponse *response = request->beginResponse_P(
            200, "text/js", main_js_gz, main_js_gz_len);
        response->addHeader("Content-Encoding", "gzip");
        request->send(response);
    });
    server.on("/cnosh.js", HTTP_GET, [](AsyncWebServerRequest *request) {
        AsyncWebServerResponse *response = request->beginResponse_P(
            200, "text/js", cnosh_js_gz, cnosh_js_gz_len);
        response->addHeader("Content-Encoding", "gzip");
        request->send(response);
    });
    server.on("/logo.png", HTTP_GET, [](AsyncWebServerRequest *request) {
        AsyncWebServerResponse *response = request->beginResponse_P(
            200, "image/png", logo_png_gz, logo_png_gz_len);
        response->addHeader("Content-Encoding", "gzip");
        request->send(response);
    });
        ...
        [&configuration, this](AsyncWebServerRequest *request) {
            ...
            for (const auto &interfaceElement : interfaceElements) {
                ...
                JsonObject &attributes =
                    element.createNestedObject("attributes");
                element["element"] =
                    _jsonBuffer.strdup(interfaceElement.element);
                element["id"] = _jsonBuffer.strdup(interfaceElement.id);
                element["content"] =
                    _jsonBuffer.strdup(interfaceElement.content);
                ...
                if (interfaceElement.getAttribute("data-config").length() !=
                    0) {
                    if (interfaceElement.getAttribute("type") == "password") {
                        attributes["placeholder"] = "Password unchanged";
                        attributes["value"] = "";
                    } else {
                        attributes["value"] = String{configuration.get(
                            interfaceElement.getAttribute("data-config"))};
                    }
                }
            }
#ifdef DEBUG
...
class. Also no const getter. As I refuse to bring that code to 21st
century, we have to live with it until someone brave fixes it.
*/
    const std::map<WebRequestMethodComposite, std::string> requestMethods{
        {HTTP_GET, "GET"},         {HTTP_POST, "POST"},
        {HTTP_DELETE, "DELETE"},   {HTTP_PUT, "PUT"},
        {HTTP_PATCH, "PATCH"},     {HTTP_HEAD, "HEAD"},
    ...
    for (int i = 0; i < request->params(); i++) {
        ...
        output << "\t";
        if (parameter->isFile()) {
            output << "This is a file. FileSize: " << parameter->size()
                   << std::endl
                   << "\t\t";
        }
...
void WebServer::addInterfaceElement(const String &id, String element,
    ...
    if (configvariable.length() != 0) {
        setInterfaceElementAttribute(id, "data-config",
                                     std::move(configvariable));
    }
}
...
void WebServer::reset() {
    ...
    // We should also reset the server itself, according to documentation, but
    // it will cause a crash. It works without reset, if you only configure one
    // server after a reboot. Not sure what happens if you want to reconfigure
    // during runtime.
    // server.reset();
    // server.addHandler(&events);
    //#ifdef BASECAMP_USEDNS
